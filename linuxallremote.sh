...
while true; do
	...
	then
		...
		Stampa " 2603. ochronasec/ochrona-cli" "1332. airbus-seclab/ilo4_toolbox" "1373. SideChannelMarvels/JeanGrey"
	...
	then
		...
		Stampa " 2545. DivineSoftware/AutoRoot" "2560. 0x1CA3/AdbNet" "1366. flankerhqd/JAADAS"
	...
	then
		...
		Stampa " 2623. omer-dogan/kali-whoami"
		echo "$SEP"
	fi
	...
	if [[ "$CAV" == "[-]" ]];
	then
		Stampa " 946. moxie0/chapcrack" "2582. sensepost/assless-chaps"
		echo "$SEP"
	fi
	...
	then
		...
		Stampa " 2532. ZecOps/CVE-2020-0796-RCE-POC" "2495. cube0x0/CVE-2021-1675" "2619. fullhunt/log4j-scan"
		Stampa " 2620. kozmer/log4j-shell-poc"
		echo "$SEP"
	fi
	echo "$CBG"" BG. D"
	if [[ "$CBG" == "[-]" ]];
	...
	then
		...
		Stampa " 1392. hotelzululima/kacak" "1423. carlospolop/legion" "2621. SecuProject/ADenum"
	...
	then
		...
		Stampa " 2555. junegunn/fzf" "1300. nccgroup/hodor" "1341. BountyStrike/Injectus"
		echo "$SEP"
	fi
	...
	then
		...
		Stampa " 2590. DedSecInside/gotor" "2592. s41r4j/phomber" "2602. xadhrit/terra"
		Stampa " 1329. BillyV4/ID-entify" "1340. m4ll0k/infoga" "1342. penafieljlm/inquisitor"
		Stampa " 1344. sc1341/InstagramOSINT" "1355. Rajkumrdusad/IP-Tracer" "2608. emrekybs/Expulso"
		Stampa " 2611. Q0120S/NoobWebHunter" "1445. initstring/linkedin2username" "1469. HurricaneLabs/machinae"
		Stampa " 1483. saeeddhqan/Maryam" "1497. j3ssie/metabigor" "2635. kennbroorg/iKy"
		Stampa " 2636. machine1337/userfinder/osint"
		echo "$SEP"
	fi
	...
	then
		...
		Stampa " 2379. kurobeats/fimap" "2640. rm1984/IMAPLoginTester"
		echo "$SEP"
	fi
	echo "$CCZ"" CZ. IMSI"
	if [[ "$CCZ" == "[-]" ]];
	...
	if [[ "$CDQ" == "[-]" ]];
	then
		Stampa " 1431. aepereyra/lfimap" "2606. kostas-pa/LFITester" "1430. OsandaMalith/LFiFreak"
	...
	then
		...
		Stampa " 2561. 0xFreddox/KeyLogger-WebService" "2588. Revise7/ViperVenom" "1478. technoskald/maltrieve"
		echo "$SEP"
	fi
	...
	then
		...
		Stampa " 2597. FunnyWolf/Viper" "2609. hpthreatresearch/subcrawl" "1428. leviathan-framework/leviathan"
		Stampa " 2612. AsjadOooO/Zero-attacker" "1460. api0cradle/LOLBAS" "1465. lateralblast/lunar"
	...
	then
		...
		Stampa " 2610. Deadpool2000/Paybag" "1412. sensepost/kwetza" "1530. g0tmi1k/mpc"
		echo "$SEP"
	fi
	echo "$CEN"" EN. PDF"
	if [[ "$CEN" == "[-]" ]];
	...
	then
		...
		Stampa " 2598. sumo2001/Trishul" "1306. gabamnml/hoper" "1307. SpiderLabs/HostHunter"
	...
	then
		...
		Stampa " 2447. octetsplicer/LAZYPARIAH" "2456. GetRektBoy724/MeterPwrShell" "2600. lawndoc/mediator"
	...
	then
		...
		Stampa " 2661. 1N3/Wordpress-XMLRPC-Brute-Force-Exploit"
		echo "$SEP"
	fi
	echo "$CFR"" FR. RSA"
	if [[ "$CFR" == "[-]" ]];
	...
	then
		...
		Stampa " 2596. d4t4s3c/Shelly" "1380. s0md3v/JShell"
		echo "$SEP"
	fi
	...
	if [[ "$CHQ" == "[-]" ]];
	then
		Stampa " 2641. CodeX-ID/Temp-mail"
		echo "$SEP"
	fi
	echo "$CGJ"" GJ. TERMUX"
	if [[ "$CGJ" == "[-]" ]];
	...
	then
		...
		Stampa " 2662. check binary's security"
		echo "$SEP"
	fi
	...
	if [[ "$CHO" == "[-]" ]];
	then
		Stampa " 2637. TheSpeedX/WhatScraper"
		echo "$SEP"
	fi
	...
	then
		...
		Stampa " 2601. V1n1v131r4/webdiscover" "1311. riramar/hsecscan" "1312. segment-srl/htcap"
		Stampa " 1317. tomnomnom/httprobe" "1318. breenmachine/httpscreenshot" "1368. stasinopoulos/jaidam"
		Stampa " 1393. P0cL4bs/Kadimus" "2607. rivalsec/pathbuster" "1414. takeshixx/laf"
		echo "$SEP"
	fi
	echo "$CGZ"" GZ. WEBDAV"
	if [[ "$CGZ" == "[-]" ]];
	...
	then
		...
		Stampa " 2544. EatonChips/wsh" "2591. oldkingcone/slopShell" "1315. wireghoul/htshells"
		echo "$SEP"
	fi
	echo "$CHC"" HC. WEBSOCKET"
	if [[ "$CHC" == "[-]" ]];
	then
		Stampa " 2624. PalindromeLabs/STEWS"
		echo "$SEP"
	fi
	...
	if [[ "$CHN" == "[-]" ]];
	then
		Stampa " 1499. hahwul/metasploit-autopwn/db_autopwn" "2663. pwndbg/pwndbg"
		echo "$SEP"
	fi
	echo "$SEP"
	...
	"365")
		...
		read -e -p "(example, background.jpg): " JPG
		if [[ -f "$JPG" ]];
		then
	...
	"2659")
		...
		read -p "(example, https://www.target.com/xmlrpc.php): " TURL
		if [[ "$TURL" != "" ]];
		then
			...
			then
				...
				read -e -p "(example, /usr/share/wordlist/rockyou.txt): " WRDL
				if [[ "$WRDL" != "" && -f "$WRDL" ]];
				then
					for PSS in $(cat "$WRDL")
					do
	...
	"2660")
		...
		if [[ "$TURL" != "" ]];
		then
			curl -X POST -d "<?xml version=\"1.0\" encoding=\"utf-8\"?><methodCall><methodName>system.listMethods</methodName><params></params></methodCall>" "$TURL"
		fi
	;;
	"2661")
